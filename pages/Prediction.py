...
# --- Forecast future input features ---
future_years = list(range(2023, 2031))
future_data = []
...
# --- Add lag feature: Crime last year ---
df_state = df_state.sort_values("Year")
df_state["Crime_Last_Year"] = df_state["Crime_Count"].shift(1)
df_state.dropna(inplace=True)

# --- Smooth Holt-forecasted inputs with artificial growth ---
future_df["Population"] *= 1 + 0.01 * np.arange(len(future_df))  # simulate growth
future_df["Income"] *= 1 + 0.005 * np.arange(len(future_df))     # slight economic trend
future_df["Unemployment_Rate"] *= 1 + 0.002 * np.arange(len(future_df))  # simulate small change

# --- Prepare features and target ---
X = df_state[["Year", "Literacy_Rate", "Unemployment_Rate", "Population", "Income", "Crime_Last_Year"]]
y = df_state["Crime_Count"]

# --- Add placeholder for Crime_Last_Year ---
last_crime_val = df_state["Crime_Count"].iloc[-1]
future_df["Crime_Last_Year"] = [last_crime_val] + [np.nan]*(len(future_df)-1)
...
for i in range(len(future_df)):
    if i == 0:
        crime_last = last_crime_val
    else:
        crime_last = predicted_crimes[-1]
    future_df.at[i, "Crime_Last_Year"] = crime_last

    # Combine for scaling
    all_features = pd.concat([X, future_df[["Year", "Literacy_Rate", "Unemployment_Rate", "Population", "Income", "Crime_Last_Year"]]])
    scaler = MinMaxScaler()
    scaler.fit(all_features)

    X_scaled = scaler.transform(X)
    future_scaled = scaler.transform(future_df[["Year", "Literacy_Rate", "Unemployment_Rate", "Population", "Income", "Crime_Last_Year"]])

    # Choose your model (select one)
    # model = RandomForestRegressor(n_estimators=200, random_state=42)
    # model = GradientBoostingRegressor(n_estimators=200, random_state=42)
    model = LinearRegression()

    model.fit(X_scaled, y)
    pred = model.predict([future_scaled[i]])[0]
    predicted_crimes.append(pred)

future_df["Predicted_Crime_Count"] = np.round(predicted_crimes).astype(int)

# --- Plot Historical + Predicted ---
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(df_state["Year"], df_state["Crime_Count"], marker='o', label="Historical")
ax.plot(future_df["Year"], future_df["Predicted_Crime_Count"], marker='s', color='red', label="Prediction")
ax.set_title(f"{selected_state} - {crime_type} Forecast (2023–2030) with Enhanced Features")
ax.set_xlabel("Year")
ax.set_ylabel("Crime Cases")
ax.legend()
st.pyplot(fig)
